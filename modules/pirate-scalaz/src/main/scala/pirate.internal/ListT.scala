...
object ListT {
  ...
  implicit def ListTInstances[F[+_]: Monad]
    : Monad[({ type l[a] = ListT[F, a] })#l] with MonadPlus[({ type l[a] = ListT[F, a] })#l] =
    new Monad[({ type l[a] = ListT[F, a] })#l] with MonadPlus[({ type l[a] = ListT[F, a] })#l] {
