...
object ParseTraversal {
  ...
  def search[F[_]: Monad, A](f: OptionRunner[F], parser: Parse[A]): NondetT[F, Parse[A]] = {
    ...
    parser match {
      ...
      case BindParse(k, a) =>
        search(f, a).flatMap(p =>
          ParseTraversal.eval(true, p) match {
    ...
    case SwitchParser(flag, _, a) =>
      w.name match {
        case ShortParsedName(c) =>
          flag.hasShort(c).option(update[A](args => ((w.value.map(s => "-" ++ s).toList ++ args) -> a).pure[P]))
      ...
      w.name match {
        case ShortParsedName(c) =>
          flag
            ...
            .option(
              update[A](args =>
                p.read(w.value.toList ++ args) match {
