...
object InterpreterSpec extends Properties {
  ...
  def requiredMissingA: Result =
    run(
      (flag[String](short('a'), Flags.empty) |@| flag[String](short('b'), Flags.empty))(_ -> _),
      List()
    ).toEither ==== Left(
      ParseErrorMissing(
        ParseTreeAp(
          List(
            ParseTreeLeaf(FlagInfo(ShortName('a'), None, None, false, false)),
            ParseTreeLeaf(FlagInfo(ShortName('b'), None, None, false, false))
          )
        )
      )
    )
  ...
  def requiredMissingB: Result =
    run(
      (flag[String](short('a'), Flags.empty) |@| flag[String](short('b'), Flags.empty))(_ -> _),
      List("-b", "c")
    ).toEither ==== Left(ParseErrorMissing(ParseTreeLeaf(FlagInfo(ShortName('a'), None, None, false, false))))

  def requiredMissingC: Result =
    run(
      (flag[String](short('a'), Flags.empty) |@| flag[String](short('b'), Flags.empty))(_ -> _),
      List("-a", "c")
    ).toEither ==== Left(ParseErrorMissing(ParseTreeLeaf(FlagInfo(ShortName('b'), None, None, false, false))))

  def leftover: Result =
    run(().pure[Parse], List("-a")) ==== ParseErrorLeftOver("-a" :: Nil).left

  def requiredMissingAlts: Result =
    run(flag[String](short('a'), Flags.empty) ||| flag[String](short('b'), Flags.empty), List()).toEither ==== Left(
      ParseErrorMissing(
        ParseTreeAlt(
          List(
            ParseTreeLeaf(FlagInfo(ShortName('a'), None, None, false, false)),
  ...
  def multipleSwitches: Result =
    run((switch(short('a'), Flags.empty) |@| switch(short('b'), Flags.empty))(_ -> _), List("-ab")) ==== (
      true,
      true
    ).right

  def flagAfterSwitch: Result =
    run((switch(short('a'), Flags.empty) |@| flag[String](short('b'), Flags.empty))(_ -> _), List("-ab", "c")) ==== (
      true,
      "c"
    ).right
  ...
  def positionalArgs: Result =
    run(
      (argument[String](metavar("src")) |@| argument[String](metavar("dst")))(_ -> _),
      List("/tmp/src", "tmp/dst")
    ) ==== ("/tmp/src", "tmp/dst").right
  ...
  def positionalFollowingMany: Property = for {
    ...
    run((argument[String](metavar("src")) |@| arguments[String](metavar("dst")))(_ -> _), "--" :: args) ==== (
      args.head,
      args.tail
    ).right
  }
  ...
  def manyFollowingPositional: Property = for {
    ...
    Result
      .assert(
        run((arguments[String](metavar("dst")) |@| argument[String](metavar("src")))(_ -> _), "--" :: args) != (
          args.init,
          args.last
        ).right
      )
  ...
  def intArgString: Result =
    run(argument[Int](metavar("src")), List("file.txt")) ==== ParseErrorMessage(
      "Error parsing `file.txt` as `Int`"
    ).left

  def missingArg: Result =
    Result.assert(run(argument[Int](metavar("src")), Nil).toEither.isLeft)

  def dobacktrack: Result = Interpreter.run(
    (subcommand(().pure[Parse] ~ "first") |@| switch(short('a'), Flags.empty))(_ -> _),
    "first" :: "-a" :: Nil,
    DefaultPrefs()
  ) ==== (("first" :: Nil) -> ((), true).right)

  def donotbacktrack: Result = Interpreter.run(
    (subcommand(().pure[Parse] ~ "first") |@| switch(short('a'), Flags.empty))(_ -> _),
    "first" :: "-a" :: Nil,
    DefaultPrefs().copy(backtrack = false)
  ) ==== (("first" :: Nil) -> ParseErrorLeftOver("-a" :: Nil).left)

  def donotbacktrackbutstillwork: Result = Interpreter.run(
    (subcommand(().pure[Parse] ~ "first") |@| switch(short('a'), Flags.empty))(_ -> _),
    "-a" :: "first" :: Nil,
    DefaultPrefs().copy(backtrack = false)
  ) ==== (("first" :: Nil) -> ((), true).right)
  ...
  def subcontext: Result = Interpreter.run(
    subcommand(subcommand(subcommand(().pure[Parse] ~ "third") ~ "second") ~ "first"),
    "first" :: "second" :: "third" :: Nil,
    DefaultPrefs()
  ) ==== (("first" :: "second" :: "third" :: Nil) -> ().right)
