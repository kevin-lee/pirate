...
"""
  def impl(n: Int)     =
    if (n <= 12)
      ...
      s"""Z.apply2(Z.apply${n / 2}(${(0 until (n / 2))
        .map(m => params(m).toLower)
        .mkString(", ")})((${(0 until (n / 2)).map(_ => "_").mkString(", ")})), Z.apply${n / 2}(${(0 until (n / 2))
        .map(m => params(m + n / 2).toLower)
        .mkString(", ")})((${(0 until (n / 2)).map(_ => "_").mkString(", ")})))((x1, x2) => fab(${(1 to (n / 2))
        .map(m => "x1._" + m)
        .mkString(", ")}, ${(1 to (n / 2)).map(m => "x2._" + m).mkString(", ")}))"""
    else
      s"""Z.apply3(Z.apply${n / 2}(${(0 until (n / 2))
        .map(m => params(m).toLower)
        .mkString(", ")})((${(0 until (n / 2)).map(_ => "_").mkString(", ")})), Z.apply${(n / 2)}(${(0 until (n / 2))
        .map(m => params(m + n / 2).toLower)
        .mkString(", ")})((${(0 until (n / 2)).map(_ => "_").mkString(", ")})), ${params(
        n - 1
      ).toLower})((x1, x2, x3) => fab(${(1 to (n / 2)).map(m => "x1._" + m).mkString(", ")}, ${(1 to (n / 2))
        .map(m => "x2._" + m)
        .mkString(", ")}, x3))"""
