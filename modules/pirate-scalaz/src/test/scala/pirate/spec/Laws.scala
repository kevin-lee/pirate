    ...
    def laws[F[_]](
      ...
      functor.laws[F](name, genFInt, genIntToInt) ++
        List(
          ...
          property(
            s"$name - $lawName: composition",
            applicative.composition[F, Int, Int, Int](genFInt, genFIntToInt, genFIntToInt)
          ),
          ...
          property(
            s"$name - $lawName: map consistent with ap",
            applicative.mapApConsistency[F, Int, Int](genFInt, genIntToInt)
          )
        )
    }
  }
    ...
    def laws[M[_]](
      ...
      applicative.laws[M](name, genInt, genIntToInt, genMInt, genMIntToInt) ++
        List(
          ...
          property(
            s"$name - $lawName: associativity",
            monad.associativity[M, Int, Int, Int](genMInt, genIntToMInt, genIntToMInt)
          ),
          property(
            s"$name - $lawName: ap consistent with bind",
            monad.bindApConsistency[M, Int, Int](genMInt, genMIntToInt)
          )
        )
    }
  }

  object traverse extends LawsCompat.traverseCompat
